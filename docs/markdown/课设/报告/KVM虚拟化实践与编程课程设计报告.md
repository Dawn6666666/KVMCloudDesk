...
1. **成员 A（后端负责人）**：
   ...
   - **底层映射与内存管理**：手动映射 57 个 libvirt 原生 C 接口和 4 个核心结构体，通过 JNA 访问底层物理动态库；设计显式释放堆外内存指针（通过 C 语言的标准 free 接口）的防内存泄漏方案。
   - **双环境隔离设计**：利用 Spring 的条件装配机制，编写 mock 与 libvirt 双 Profile 服务层实现类，保障在 Windows 本地开发时能正常返回假数据调优，在 CentOS 下则直接对接真实的 KVM 平台。
   - **服务部署与问题排查**：完成项目在 CentOS 虚拟机环境的打包部署，排查并解决了由本地 QEMU 旧版程序冲突、DNS 域名反解超时引起的服务连接延迟。

2. **成员 B（前端与 Swing 负责人）**：
   - **客户端构建与界面开发**：负责 client-web 模块的页面搭建，基于 Vue 3、TypeScript 与 Element Plus 设计统一的管理视图；开发基于 Swing 的桌面客户端，使用 FlatLaf 主题库美化 UI 控件。
   - **控制台看板与日志追踪**：在前端中集成 ECharts 数据可视化图表，展示物理机的实时资源负载；结合 Axios 拦截器与 Pinia 状态管理，通过 X-Action-Description 请求头追踪用户的操作轨迹。
   - **时序逻辑与状态控制**：编写虚拟机安全关机状态的 1.5 秒异步状态轮询器，实现了当 ACPI 关机信号超时后自愈降级到“强制断电”的操作处理逻辑。
   - **联调测试与文档编写**：负责多端与后端的联合调试工作，编写所有的宿主机与虚拟机资源调测用例，并主导完成了课程设计报告的文案与时序图设计。
...
gantt
    ...
    section 成员 A
    后端接口设计       :a1, 2026-06-18, 2d
    双模式实现         :a2, after a1, 2d
    JNA 与部署         :a3, after a2, 2d
    接口排查           :a4, after a3, 2d

    section 成员 B
    页面与接口         :b1, 2026-06-18, 3d
    日志与轮询         :b2, after b1, 2d
    图表与联调         :b3, after b2, 2d
    测试与报告         :b4, after b3, 1d
```
